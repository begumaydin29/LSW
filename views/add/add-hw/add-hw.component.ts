...
import { AddHwConfirmComponent } from './popup/add-hw-confirm/add-hw-confirm.component';
import { AcRegSearchProxyService } from '@app/lsw/services/lookup-search-services/acReg-search/acReg-search-proxy.service';

@Component({
  selector: 'app-add-hw',
  templateUrl: './add-hw.component.html',
  styleUrls: ['./add-hw.component.css'],
})
export class AddHwComponent implements OnInit {
  form: FormGroup;
  ...
  async authorizedACRegChanged($event) {
    ...
    if ($event && $event.length > searchBeginNumber) {
      ...
      searchModel_key.fieldName = 'TailNumber';
  ...
  selectErd($event) {
    this.erd.valueChanges.subscribe(async (change) => {
      if (change != null) {
        const result = await this.authorizedErdDesc();
  ...
  async authorizedErdDesc() {
    const result = await this.erdDescriptionProxyService
      .getErdDescriptionByErd(this.erd.value)
      .toPromise();
