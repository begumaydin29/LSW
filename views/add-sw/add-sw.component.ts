...
import { AddSwConfirmComponent } from "./popup/add-sw-confirm/add-sw-confirm.component";
...
import { AddHwModalComponent } from "./popup/add-hw-modal/add-hw-modal.component";

@Component({
  selector: "app-add-sw",
  templateUrl: "./add-sw.component.html",
  styleUrls: ["./add-sw.component.css"]
})
export class AddSwComponent implements OnInit {
  @ViewChild("confirm", { static: false })
  addSwConfirmModal: AddSwConfirmComponent;

  @ViewChild("addHw", { static: false })
  addHwModalComponent: AddHwModalComponent;
  ...
  constructor(
    ...
    private partNumberProxyService: PartNumberProxyService,
    private partNumberDescriptionProxyService: PartNumberDescriptionProxyService,
    private swLocationValueProxyService: SwLocationValueProxyService,
    private equipmentIdProxyService: EquipmentIdProxyService,
    private ataChapterProxyService: AtaChapterProxyService,
    private swLocationDescriptionProxyService: SwLocationDescriptionProxyService,
    private checkErdProxyService: CheckErdProxyService,
    private airplaneByIdProxyService: AirplaneByIdProxyService,
    private checkHwPnProxyService: CheckHwPnProxyService,
    private checkSlidProxyService: CheckSlidProxyService
  ) { }
  ...
  setLogSuccess(succesfulInsertedAirplaneIds) {
    console.log("success", succesfulInsertedAirplaneIds);
  ...
  setLogError(errorInsertedAirplaneIds) {
    console.log("error", errorInsertedAirplaneIds);
  ...
  async authorizedERDChanged($event) {
    ...
    if ($event && $event.length > searchBeginNumber) {
      let result = await this.equipmentProxyService
        .postByAircraftId(this.holdAirplaneIds)
        .toPromise();
      ...
      let nullDescriptionEquipmentList = this.equipmentList.filter(function (
        element
      ) {
      ...
      let equipmentListWoutNull = this.equipmentList.filter(
        o =>
          !nullDescriptionEquipmentList.find(
            o2 => o["Equipment_Id"] === o2["Equipment_Id"]
          )
      );

      let equipmentListWoutDupl = this.removeDuplicatesBy(
        x => x.ReferenceDesignator,
        equipmentListWoutNull
      );

      this.filteredEquipmentList = this.insertToEquipmentListWoutNull(
        equipmentListWoutDupl,
        this.removeDuplicatesBy(
          x => x.ReferenceDesignator,
          nullDescriptionEquipmentList
        )
      );
    }
  }
  ...
  async checkErd() {
    ...
    if (this.erd.value != null) {
      ...
      let result_ = await this.checkErdProxyService
        .checkErd(this.holdAirplaneIds, referenceDesignator)
        .toPromise();
      ...
      if (this.invalidtcValuesErd.length !== 0) {
        ...
        this.erdErrorMessage =
          referenceDesignator + "(ERD) not found on " + message;
      }
  ...
  async getAirplaneById(id: number): Promise<any> {
    let result = await this.airplaneByIdProxyService
      .getAirplaneById(id)
      .toPromise();
  ...
  async checkHwPn() {
    ...
    if (this.hw.value != null) {
      ...
      const result_: any = await this.checkHwPnProxyService
        .checkHwPn(this.holdAirplaneIds, referenceDesignator, partNumber)
        .toPromise();
  ...
  findEquipmentId(partNumber: string, erd: string) { //findEquipmentId

    console.log('jsj', partNumber);
    console.log('jdjd', erd);

    this.equipmentIdProxyService
      .getEquipmentId(partNumber, erd)
      .subscribe(response => {
        let equipmentId = response.Result;

        this.ataChapterProxyService
          .getAtaChapter(equipmentId)
          .subscribe(response => {
            if (response.Success) {
  ...
  getPartNumberDescription(airplaneIdList, erd, partNumber: PartNumberDto[]) {
    this.hwPnList = [];

    -    partNumber.forEach(element => {
      -      this.partNumberDescriptionProxyService
        .getPartNumberDescription(element)
        .subscribe(response => {
          if (response.Result.length === 0) {
            this.hwPnList.push(element + ": -");
          } else {
            this.hwPnList.push(element + ": " + response.Result[0]);
          }
        });
    });
  }
  ...
  async authorizedSlidChanged($event) {
    ...
    if ($event && $event.length > searchBeginNumber) {
      let result = await this.swLocationValueProxyService
        .getByValue($event)
        .toPromise();

      result.Result.forEach(element => {
        this.swLocationDescriptionProxyService
          .getBySlid(element)
          .subscribe(response => {
            if (response.Result["Description"] === "") {
              if (!this.swLocationList.includes(element + ": -"))
                this.swLocationList.push(element + ": -");
            } else {
              if (!this.swLocationList.includes(element + ": " + response.Result["Description"]))
                this.swLocationList.push(
                  element + ": " + response.Result["Description"]
                );
            }
          });
      });
  ...
  async checkSlid() {
    ...
    if (this.slid.value != null) {
      ...
      const result_: any = await this.checkSlidProxyService
        .checkSlid(this.holdAirplaneIds, slid)
        .toPromise();
  ...
  filterValidTcValues() {
    for (let i = 0; i < this.allInvalidTcValues.length; i++) {
      let val = this.allInvalidTcValues[i];
      let foundIndex = this.holdAirplaneList.indexOf(val);
      if (foundIndex != -1) {
  ...
  save() {
    ...
    let ataCh = this.ataChValue;
    let slid = this.slid.value.substr(0, this.slid.value.indexOf(":")).toUpperCase();
    let slidDescription = this.slid.value.substr(
      this.slid.value.indexOf(":") + 1
    ).toUpperCase();
